...
CREATE OR REPLACE FUNCTION public.audit_trigger_function_admin() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
    DECLARE
        current_member TEXT;
...
CREATE TABLE IF NOT EXISTS public.audit_log_admins (
    id bigint NOT NULL,
...
CREATE SEQUENCE IF NOT EXISTS public.audit_log_admins_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE IF NOT EXISTS public.pay_details (
    id bigint NOT NULL,
...
CREATE SEQUENCE IF NOT EXISTS public.pay_details_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE IF NOT EXISTS public.pay_tenants (
    id bigint NOT NULL,
    tenant_id bigint NOT NULL,
    admin_id bigint NOT NULL,
...
CREATE SEQUENCE IF NOT EXISTS public.pay_tenants_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE IF NOT EXISTS public.setting_tenants (
    id bigint NOT NULL,
    tenant_id bigint NOT NULL,
...
CREATE SEQUENCE IF NOT EXISTS public.setting_tenants_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE INDEX IF NOT EXISTS idx_admins_deleted_at ON public.admins USING btree (deleted_at);
CREATE UNIQUE INDEX IF NOT EXISTS idx_admins_username ON public.admins USING btree (username);
CREATE INDEX IF NOT EXISTS idx_audit_log_admins_transaction_id ON public.audit_log_admins USING btree (transaction_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_credentials_tenant_id ON public.credentials USING btree (tenant_id);
CREATE INDEX IF NOT EXISTS idx_modules_deleted_at ON public.modules USING btree (deleted_at);
CREATE UNIQUE INDEX IF NOT EXISTS idx_modules_name ON public.modules USING btree (name);
CREATE UNIQUE INDEX IF NOT EXISTS idx_plans_name ON public.plans USING btree (name);
CREATE UNIQUE INDEX IF NOT EXISTS idx_setting_tenants_tenant_id ON public.setting_tenants USING btree (tenant_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_tenant_module ON public.tenant_modules USING btree (module_id, tenant_id);
CREATE INDEX IF NOT EXISTS idx_tenant_modules_deleted_at ON public.tenant_modules USING btree (deleted_at);
CREATE UNIQUE INDEX IF NOT EXISTS idx_tenants_cuit_pdv ON public.tenants USING btree (cuit_pdv);
CREATE INDEX IF NOT EXISTS idx_tenants_deleted_at ON public.tenants USING btree (deleted_at);
CREATE UNIQUE INDEX IF NOT EXISTS idx_tenants_email ON public.tenants USING btree (email);
...
DO $$ BEGIN
    CREATE TRIGGER tr_update_credentials BEFORE UPDATE ON public.credentials FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
EXCEPTION WHEN duplicate_object OR sqlstate '42P07' OR sqlstate '42P16' THEN NULL; END $$;
