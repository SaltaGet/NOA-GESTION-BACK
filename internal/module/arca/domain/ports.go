...
type ArcaRepository interface {
	...
	SaveInvoice(factura *schemas.FacturaElectronica, incomeSaleID int64) error
}

type ArcaService interface {
	EmitInvoice(user *schemas.AuthenticatedUser, pointSaleID int64, incomeSaleID int64, req *schemas.FacturaRequest, isHomo bool) (*schemas.FacturaElectronica, error)
}
