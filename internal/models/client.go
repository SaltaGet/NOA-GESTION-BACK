package models

import (
	"time"

	"gorm.io/gorm"
)

type Client struct {
	ID                     int64          `gorm:"primaryKey;autoIncrement" json:"id"`
	FirstName              string         `gorm:"not null;type:varchar(30)" json:"first_name"`
	LastName               string         `gorm:"not null;type:varchar(30)" json:"last_name"`
	CompanyName            *string        `gorm:"type:varchar(255)" json:"company_name"`
	Identifier             *string        `gorm:"unique;type:varchar(20)" json:"cuit,omitempty"`
	Email                  *string        `gorm:"unique;type:varchar(100)" json:"email" validate:"omitempty,email"`
	Phone                  *string        `gorm:"type:varchar(20)" json:"phone"`
	Address                *string        `gorm:"type:varchar(255)" json:"address"`
	ResponsabilityFrontIVA *string        `gorm:"type:varchar(255)" json:"responsability_front_iva"`
	MemberCreateID         int64          `gorm:"not null" json:"member_create_id"`
	MemberCreate           Member         `gorm:"foreignKey:MemberCreateID" json:"member_create"`
	Pay                    []PayIncome    `gorm:"foreignKey:ClientID" json:"pay"`
	CreatedAt              time.Time      `gorm:"autoCreateTime" json:"created_at"`
	UpdatedAt              time.Time      `gorm:"autoUpdateTime" json:"updated_at"`
	DeletedAt              gorm.DeletedAt `gorm:"index" json:"deleted_at"`
}
